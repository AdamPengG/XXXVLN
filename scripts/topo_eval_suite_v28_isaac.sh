...
AUDIT_DIR="${RUN_DIR}/asset_audit"
ASSETS_LOCAL_DIR="${RUN_DIR}/assets_local"
MASTER_LOG="${LOG_DIR}/topo_eval_suite_v28_isaac.log"
CFG_PATH="${ROOT_DIR}/configs/isaac_scenes_v28.yaml"
ASSETS_ROOT="${ISAAC_ASSETS_ROOT:-/home/peng/isaacsim_assets}"
STAGE="${ISAAC_STAGE_USD:-${ASSETS_ROOT}/Assets/Isaac/5.1/Isaac/Environments/Office/office.usd}"
SCENE_ID="office_localized"

mkdir -p "${RUN_DIR}" "${BUILD_DIR}" "${LOG_DIR}" "${DEBUG_ROOT}" "${AUDIT_DIR}" "${ASSETS_LOCAL_DIR}"
: > "${MASTER_LOG}"
...
RUN_LIMIT="${TOPO_V28_RUN_LIMIT:-3}"
if [ "${small}" = "1" ]; then
  RUN_LIMIT=1
fi
...
echo "[V28_ISAAC_SUITE] start=$(date -Iseconds) run_dir=${RUN_DIR}" | tee -a "${MASTER_LOG}"
...
echo "[V28_ISAAC_SUITE] asset_audit_begin" | tee -a "${MASTER_LOG}"
(
...
echo "[V28_ISAAC_SUITE] asset_localize_begin" | tee -a "${MASTER_LOG}"
(
  bash scripts/gpu/run_isaac_on_5090.sh -- \
    bash scripts/isaac/asset_localize_v28.sh \
      --stage "${STAGE}" \
      --assets_root "${ASSETS_ROOT}" \
      --out_root "${ASSETS_LOCAL_DIR}" \
      --allow_outside_root 0
) 2>&1 | tee -a "${MASTER_LOG}"

LOCAL_STAGE="$(
  python3 - <<'PY' "${ASSETS_LOCAL_DIR}/manifest_copied.json"
import json,sys
d=json.load(open(sys.argv[1]))
print(str(d.get("stage_localized","")))
PY
)"
if [ -z "${LOCAL_STAGE}" ] || [ ! -f "${LOCAL_STAGE}" ]; then
  echo "[V28_ISAAC_SUITE] localized_stage_missing=${LOCAL_STAGE}" | tee -a "${MASTER_LOG}"
  exit 3
fi

echo "[V28_ISAAC_SUITE] render_smoke_begin stage=${LOCAL_STAGE}" | tee -a "${MASTER_LOG}"
(
  ISAAC_SMOKE_RUN_DIR="${RUN_DIR}" \
  ISAAC_CFG="${CFG_PATH}" \
  ISAAC_SCENE_ID="${SCENE_ID}" \
  ISAAC_STAGE_USD="${LOCAL_STAGE}" \
  bash scripts/isaac/render_smoke_v27.sh --stage "${LOCAL_STAGE}"
) 2>&1 | tee -a "${MASTER_LOG}"
...
graph = {"meta": {"scene_id": scene, "backend": "isaac_v28"}, "nodes": nodes, "edges": edges}
...
summary_json="${RUN_DIR}/v28_summary_tmp.json"
...
echo "[TOPO_EVAL_SUMMARY_V28_ISAAC] scene=${SCENE_ID} runs=${runs} success=${succ} renderer=${renderer}" | tee -a "${MASTER_LOG}"
