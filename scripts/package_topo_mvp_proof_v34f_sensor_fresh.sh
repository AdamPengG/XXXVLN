...
TS="$(date +%Y%m%d_%H%M)"
ZIP="/home/peng/DualVLN/forGPT/v34f_sensor_fresh_PROOF_${TS}.zip"
STAGE_DIR="/tmp/v34f_sensor_fresh_proof_${TS}"
...
for prefix, keep in (('rgb_', 80), ('depth_', 80), ('overlay_rgb_', 20)):
...
cp scripts/package_topo_mvp_proof_v34f_sensor_fresh.sh "${STAGE_DIR}/evidence/src_snapshot/"
cp scripts/tools/check_capture_motion.py "${STAGE_DIR}/evidence/src_snapshot/"
cp scripts/tools/check_capture_motion.sh "${STAGE_DIR}/evidence/src_snapshot/"
