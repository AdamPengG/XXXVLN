...
RUN_DIR="${ROOT_DIR}/runs/topo_mvp/v22_isaac"
BUILD_DIR="${RUN_DIR}/builds"
LOG_DIR="${RUN_DIR}/logs"
DEBUG_ROOT="${RUN_DIR}/debug_runs"
CFG_PATH="${ROOT_DIR}/configs/isaac_scenes_v22.yaml"
MASTER_LOG="${LOG_DIR}/topo_eval_suite_v22_isaac.log"
...
echo "[TOPO_V22] cfg=${CFG_PATH}" | tee -a "${MASTER_LOG}"
...
SCENE_LIMIT="${TOPO_V22_SCENE_LIMIT:-1}"
RUN_LIMIT="${TOPO_V22_RUN_LIMIT:-2}"
START_LIMIT="${TOPO_V22_START_LIMIT:-2}"
QUERY_LIMIT="${TOPO_V22_QUERY_LIMIT:-2}"
CONDITIONS="${TOPO_V22_CONDITIONS:-gt_pose,est_pose_noise}"
MAX_STEPS="${TOPO_V22_MAX_STEPS:-120}"
...
graph = {"meta": {"scene_id": scene, "backend": "isaac_v22"}, "nodes": nodes, "edges": edges}
...
for cond in "${conds[@]}"; do
  ...
  cond_log="${LOG_DIR}/topo_eval_suite_v22_isaac_${cond}.log"
  rm -rf "${cond_dir}"
  mkdir -p "${cond_dir}"
  chmod -R a+rwx "${RUN_DIR}" 2>/dev/null || true
  : > "${cond_log}"
...
print(
    f"[TOPO_EVAL_SUMMARY_V22_ISAAC] backend={backend_mode} condition={cond} "
...
json.dump(out, open(os.path.join(run_dir, "v22_isaac_summary.json"), "w"), indent=2)
print(f"[V22_ISAAC] wrote {os.path.join(run_dir, 'v22_isaac_summary.json')}")
PY
