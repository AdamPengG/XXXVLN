...
CFG_PATH="${ROOT_DIR}/configs/isaac_scenes_v24.yaml"
MASTER_LOG="${LOG_DIR}/topo_eval_suite_v24_isaac.log"
...
export ISAAC_SKIP_WORLD_STEP="${ISAAC_SKIP_WORLD_STEP:-1}"
export ISAAC_RENDER="${ISAAC_RENDER:-0}"
export ISAAC_MINIMAL="${ISAAC_MINIMAL:-1}"
...
export TOPO_DEBUG_ORACLE_ON_FAIL="${TOPO_DEBUG_ORACLE_ON_FAIL:-2}"

: > "${MASTER_LOG}"

echo "[TOPO_V24] cfg=${CFG_PATH}" | tee -a "${MASTER_LOG}"
...
SCENE_LIMIT="${TOPO_V24_SCENE_LIMIT:-1}"
RUN_LIMIT="${TOPO_V24_RUN_LIMIT:-1}"
START_LIMIT="${TOPO_V24_START_LIMIT:-1}"
QUERY_LIMIT="${TOPO_V24_QUERY_LIMIT:-1}"
CONDITIONS="${TOPO_V24_CONDITIONS:-gt_pose,est_pose_noise}"
MAX_STEPS="${TOPO_V24_MAX_STEPS:-120}"
...
cfg = json.loads(sys.argv[1])
lim = int(sys.argv[2])
rows = cfg.get("global", {}).get("queries", [])
if lim > 0:
    rows = rows[:lim]
for r in rows:
...
graph = {"meta": {"scene_id": scene, "backend": "isaac_v24"}, "nodes": nodes, "edges": edges}
...
for cond in "${conds[@]}"; do
  ...
  cond_log="${LOG_DIR}/topo_eval_suite_v24_isaac_${cond}.log"
  rm -rf "${cond_dir}"
  mkdir -p "${cond_dir}"
  chmod -R a+rwx "${RUN_DIR}" 2>/dev/null || true
  : > "${cond_log}"
...
print(
    f"[TOPO_EVAL_SUMMARY_V24_ISAAC] backend={backend_mode} condition={cond} "
...
json.dump(out, open(os.path.join(run_dir, "v24_isaac_summary.json"), "w"), indent=2)
print(f"[V24_ISAAC] wrote {os.path.join(run_dir, 'v24_isaac_summary.json')}")
PY
