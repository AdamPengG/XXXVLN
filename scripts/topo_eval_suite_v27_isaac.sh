...
MASTER_LOG="${LOG_DIR}/topo_eval_suite_v27_isaac.log"
CFG_PATH="${ISAAC_CFG:-${ROOT_DIR}/configs/isaac_scenes_v27.yaml}"
SCENE_ID="${ISAAC_SCENE_ID:-isaac_office_min}"
...
RUN_LIMIT="${TOPO_V27_RUN_LIMIT:-3}"
if [ "${small}" = "1" ]; then
  RUN_LIMIT=1
fi
MAX_STEPS="${TOPO_V27_MAX_STEPS:-60}"
...
echo "[V27_ISAAC_SUITE] start=$(date -Iseconds) run_dir=${RUN_DIR}" | tee -a "${MASTER_LOG}"
...
echo "[V27_ISAAC_SUITE] render_smoke_begin" | tee -a "${MASTER_LOG}"
(
...
graph = {"meta": {"scene_id": scene, "backend": "isaac_v27"}, "nodes": nodes, "edges": edges}
...
python3 - "${scene_out}" > "${RUN_DIR}/summary_tmp.json" <<'PY'
import glob
import json
import os
import sys
...
rows = []
for p in sorted(glob.glob(os.path.join(sys.argv[1], "RESULT_*.json"))):
    try:
        rows.append(json.load(open(p)))
    except Exception:
        continue
succ = sum(1 for r in rows if bool(r.get("success", False)))
print(json.dumps({"runs": len(rows), "success": succ}))
PY

summary_runs="$(python3 - <<'PY' "${RUN_DIR}/summary_tmp.json"
import json,sys
d=json.load(open(sys.argv[1]))
print(int(d.get("runs",0)))
PY
)"
summary_succ="$(python3 - <<'PY' "${RUN_DIR}/summary_tmp.json"
import json,sys
d=json.load(open(sys.argv[1]))
print(int(d.get("success",0)))
PY
)"
rm -f "${RUN_DIR}/summary_tmp.json"
...
python3 - "${DEBUG_ROOT}" <<'PY' | tee -a "${MASTER_LOG}"
import glob, json, os, sys
root = sys.argv[1]
meta_files = sorted(glob.glob(os.path.join(root, "**", "meta.json"), recursive=True))
rgb_ok = 0
total = 0
...
for p in meta_files:
    try:
        ...
        continue
    stats = m.get("rgb_capture_stats", {}) if isinstance(m.get("rgb_capture_stats"), dict) else {}
    ok = int(stats.get("rgb_ok", 0))
    ph = int(stats.get("rgb_placeholder", 0))
    if ok + ph > 0:
        total += (ok + ph)
        rgb_ok += ok
    if renderer == "unknown":
        renderer = str(m.get("renderer_used", "unknown"))
if total > 0:
    placeholder_ratio = float(max(0, total - rgb_ok) / total)
flag = 1 if (total > 0 and placeholder_ratio < 0.8 and rgb_ok > 0) else 0
print(f"[ISAAC_RGB] ok={flag} rgb_ok={rgb_ok} total={total} placeholder_ratio={placeholder_ratio:.3f} renderer_used={renderer}")
PY

echo "[TOPO_EVAL_SUMMARY_V27_ISAAC] condition=${cond} runs=${summary_runs} success=${summary_succ} renderer=${ISAAC_RENDERER}" | tee -a "${MASTER_LOG}"
