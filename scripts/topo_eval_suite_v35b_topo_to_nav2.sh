#!/usr/bin/env bash
# topo_eval_suite_v35b_topo_to_nav2.sh — hardened v35b with V35B_ anchors,
# small evidence (key frames only), safe GIF gate, overlay frames.
set -euo pipefail
...
SMALL_MODE=0
if [ "${1:-}" = "--small" ]; then SMALL_MODE=1; fi

TS="$(date +%Y%m%d_%H%M%S)"
OUT_ROOT="${V35B_OUT_ROOT:-runs/topo_mvp/v35b_topo_to_nav2_${TS}}"
...
SUITE_LOG="${LOG_DIR}/topo_eval_suite_v35b.log"
: > "${SUITE_LOG}"
...
if [ ! -f "${STAGE_OFFICIAL}" ]; then
  echo "[V35B_STAGE_VERIFY] ok=0 stage_url=${STAGE_OFFICIAL} exists=0 source=official_assets reason=missing" | tee -a "${SUITE_LOG}"
  exit 2
fi
export ISAAC_STAGE_USD="${STAGE_OFFICIAL}"
echo "[V35B_STAGE_VERIFY] ok=1 stage_url=${STAGE_OFFICIAL} exists=1 source=official_assets" | tee -a "${SUITE_LOG}"
...
export V34D_NAV2_CONTAINER="${V34D_NAV2_CONTAINER:-v35b_nav2_stack}"
export V34I_ODOM_TF_CONTAINER="${V34I_ODOM_TF_CONTAINER:-v35b_odom_tf_bridge}"
...
fail_suite() {
  local reason="$1"
  echo "[V35B_SUITE_OK] ok=0 rooms=${ROOM_COUNT:-0} doors=${DOOR_COUNT:-0} staged_succeeded=${STAGED_SUCCEEDED:-0} root=${OUT_ROOT} reason=${reason}" | tee -a "${SUITE_LOG}"
  exit 2
}
...
# ── Staged task runner (send goals + evidence) ──
run_stage_task() {
  local task="$1"
  local task_id="$2"
  local room="$3"
  ...
  if ! python3 scripts/topo_instruction_to_goal_v35a.py \
    ...
    # Object goal not found is a skip, not a hard fail
    if [ "${task}" = "object" ] && grep -q "object_not_found\|catalog.*empty\|no.*match" "${plan_log}"; then
      echo "[V35B_CASE_DONE] case=${task_id} overall=SKIPPED stages_succeeded=0/0 reason=object_not_found" | tee -a "${SUITE_LOG}"
  ...
  # Print stage plan anchor
  local stages_k
  stages_k="$(python3 - <<'PY' "${ROOMQA_DIR}/staged_goals_${task_id}.json"
import json,sys
g=json.load(open(sys.argv[1],encoding='utf-8'))
print(len(g.get('goals',[])))
PY
)"
  echo "[V35B_STAGE_PLAN] ok=1 case=${task_id} stages=${stages_k}" | tee -a "${SUITE_LOG}"
  ...
  docker exec "${V34D_NAV2_CONTAINER}" bash -lc \
    "source /opt/ros/humble/setup.bash && export ROS_DOMAIN_ID=${ROS_DOMAIN_ID} RMW_IMPLEMENTATION=${RMW_IMPLEMENTATION} && python3 /home/peng/DualVLN/scripts/nav2/send_staged_goals_v35a.py --goals_json ${goals_json} --frame map --timeout_s ${V35B_SEG_TIMEOUT_S:-90} --dist_tol_m ${V35B_SEG_DIST_TOL_M:-0.45} --yaw_tol_deg ${V35B_SEG_YAW_TOL_DEG:-40} --result_json ${seg_json}" \
    > "${seg_log}" 2>&1 || true
  cat "${seg_log}" | tee -a "${SUITE_LOG}"

  # Print V35B per-stage anchors
...
try:
    ...
    print(f"[V35B_CASE_DONE] case={tid} overall=PARSE_ERROR stages_succeeded=0/0")
    sys.exit(0)
segs=r.get('segments',[])
ok_count=sum(1 for s in segs if s.get('ok',0)==1)
for i,s in enumerate(segs):
    print(f"[V35B_NAV2_STAGE] case={tid} stage={i} status={s.get('status','?')} dist_m={s.get('dist_m','-'):.3f} yaw_err_deg={s.get('yaw_err_deg','-'):.1f} ok={s.get('ok',0)}")
overall="SUCCEEDED" if r.get('ok',0)==1 else "FAILED"
print(f"[V35B_CASE_DONE] case={tid} overall={overall} stages_succeeded={ok_count}/{len(segs)}")
PY
...
echo "[V35B_SUITE] start=$(date -Iseconds) root=${OUT_ROOT} small=${SMALL_MODE}" | tee -a "${SUITE_LOG}"
...
# Extract occ_ratio for V35B anchor
...
free=(im>200).sum(); occ=(im<50).sum(); total=im.size
print(f"{occ/total:.4f}")
PY
)"
echo "[V35B_MAP_GEN] ok=1 map_yaml=${MAP_DIR}/office_map.yaml occ_ratio=${OCC_RATIO}" | tee -a "${SUITE_LOG}"

# ── 2) Room/Topo extraction ──
...
import json,sys; obj=json.load(open(sys.argv[1],encoding='utf-8')); print(len(obj.get('rooms',[])))
PY
)"
DOOR_COUNT="$(python3 - <<'PY' "${ROOMQA_DIR}/doorway_edges.json"
import json,sys; obj=json.load(open(sys.argv[1],encoding='utf-8')); print(len(obj.get('edges',[])))
PY
)"
echo "[V35B_ROOM_PARTITION] ok=1 rooms=${ROOM_COUNT} method=occ_cc+watershed" | tee -a "${SUITE_LOG}"
echo "[V35B_DOORWAY_DETECT] ok=1 doors=${DOOR_COUNT}" | tee -a "${SUITE_LOG}"

# ── 3) Isaac ROS2 bridge probe ──
READY_FILE="${OUT_ROOT}/bridge_ready_v35b.json"
rm -f "${READY_FILE}"
: > "${LOG_DIR}/isaac_probe_v35b.log"
CAPTURE_STEPS="${V35B_CAPTURE_STEPS:-6000}"
if [ "${SMALL_MODE}" = "1" ]; then CAPTURE_STEPS="${V35B_CAPTURE_STEPS:-3000}"; fi
(
  bash scripts/gpu/run_isaac_on_5090.sh -- \
    env ISAAC_HEADLESS="${V35B_HEADLESS:-1}" ROS_DOMAIN_ID="${ROS_DOMAIN_ID}" RMW_IMPLEMENTATION="${RMW_IMPLEMENTATION}" \
      ...
      --headless "${V35B_HEADLESS:-1}" \
      --idle_action "${V35B_IDLE_ACTION:-stop}" \
      --cam_pitch_deg "${ISAAC_CAMERA_PITCH_DEG}"
) > "${LOG_DIR}/isaac_probe_v35b.log" 2>&1 &
PROBE_PID=$!
...
if [ ! -f "${READY_FILE}" ]; then
  cat "${LOG_DIR}/isaac_probe_v35b.log" >> "${SUITE_LOG}" || true
...
SAMPLE_LOG="${LOG_DIR}/sample_reachable_goals_v35b.log"
: > "${SAMPLE_LOG}"
docker exec "${V34D_NAV2_CONTAINER}" bash -lc \
  "source /opt/ros/humble/setup.bash && export ROS_DOMAIN_ID=${ROS_DOMAIN_ID} RMW_IMPLEMENTATION=${RMW_IMPLEMENTATION} && python3 /home/peng/DualVLN/scripts/nav2/sample_reachable_goals_v34j.py --map_yaml ${V34D_MAP_YAML} --out_json ${REACHABLE_JSON} --count 6 --radius_min ${V35B_RADIUS_MIN:-0.8} --radius_max ${V35B_RADIUS_MAX:-1.8} --seed ${V35B_GOAL_SEED:-3}" \
  > "${SAMPLE_LOG}" 2>&1 || true
...
# ── 9) Re-run room/topo with trajectory ──
if [ -f "${CAP_DIR}/pose_trace.csv" ]; then
  if ! run_and_log python3 scripts/topo_room_from_occ_v35a.py --map_yaml "${MAP_DIR}/office_map.yaml" --out_dir "${ROOMQA_DIR}" --pose_trace "${CAP_DIR}/pose_trace.csv"; then
    fail_suite "room_partition_with_trace_failed"
  fi
fi
...
rgbs = sorted(cap.glob("rgb_*.png"))
if not rgbs:
    print("[V35B_EVIDENCE_FRAMES] ok=0 reason=no_frames")
    sys.exit(0)
# Pick indices: 0, 10, 20, 40, 60, last
n = len(rgbs)
picks = sorted(set([0, min(10,n-1), min(20,n-1), min(40,n-1), min(60,n-1), n-1]))
copied = 0
for idx in picks:
    src = rgbs[idx]
    shutil.copy2(src, ev / f"key_{idx:05d}_{src.name}")
    # Also copy overlay if exists
    ovl = cap / src.name.replace("rgb_", "overlay_rgb_")
    if ovl.exists():
        shutil.copy2(ovl, ev / f"key_{idx:05d}_{ovl.name}")
    copied += 1
print(f"[V35B_EVIDENCE_FRAMES] ok=1 total_frames={n} key_frames={copied}")
PY

# ── 11) Safe GIF per case ──
for case in door_to_door room_center object_goal; do
  run_and_log python3 scripts/tools/make_gif_safe.py \
    --capture_dir "${CAP_DIR}" \
    --out "${EVIDENCE_DIR}/${case}_motion.gif" \
    --stride 5 \
    --min_frames 20 \
    --max_width 480 \
    --glob "rgb_*.png" || echo "[V35B_GIF_WARN] case=${case} reason=gif_creation_failed" | tee -a "${SUITE_LOG}"
done

# ── 12) Motion gate ──
if ! run_and_log bash scripts/tools/check_capture_motion.sh --capture_dir "${CAP_DIR}" --tag v35b_topo_to_nav2 --write_gif 1; then
...
echo "[V35B_SUITE_OK] ok=1 cases=${STAGED_TOTAL} succeeded=${STAGED_SUCCEEDED} skipped=${STAGED_SKIPPED} rooms=${ROOM_COUNT} doors=${DOOR_COUNT} root=${OUT_ROOT}" | tee -a "${SUITE_LOG}"
echo "[V35B_SUITE_LOG] path=${SUITE_LOG}" | tee -a "${SUITE_LOG}"
