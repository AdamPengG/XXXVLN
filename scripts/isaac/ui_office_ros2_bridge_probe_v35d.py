...
def _depth_view_sanity(obs: Any, label: str = "fp") -> dict:
    """3-ROI depth sanity: top_center, center, bottom_center.
    For a forward-looking FP camera: d_bottom < d_mid <= d_top (floor is closer).
    """
    result = {"finite_ratio": 0.0, "d_top": 0.0, "d_mid": 0.0, "d_bottom": 0.0, "ok": 0}
    ...
    if d.size == 0:
        return result
    h, w = d.shape
    finite_mask = np.isfinite(d) & (d > 0.01)
    result["finite_ratio"] = float(np.mean(finite_mask.astype(np.float32)))
    # x-range: centre 10%
    xl, xr = int(0.45 * w), int(0.55 * w)
    # top: y 5%-15%
    top = d[int(0.05 * h):int(0.15 * h), xl:xr]
    tv = top[np.isfinite(top) & (top > 0.01)]
    result["d_top"] = float(np.median(tv)) if tv.size > 0 else 0.0
    # mid: y 45%-55%
    mid = d[int(0.45 * h):int(0.55 * h), xl:xr]
    mv = mid[np.isfinite(mid) & (mid > 0.01)]
    result["d_mid"] = float(np.median(mv)) if mv.size > 0 else 0.0
    # bottom: y 80%-90%
    bot = d[int(0.80 * h):int(0.90 * h), xl:xr]
    bv = bot[np.isfinite(bot) & (bot > 0.01)]
    result["d_bottom"] = float(np.median(bv)) if bv.size > 0 else 0.0
    # Gate: ensure forward-looking camera sees reasonable geometry
    ok = 1
    if result["finite_ratio"] < 0.85:
        ok = 0
    if not (0.15 <= result["d_mid"] <= 50.0):
        ok = 0
    # At least one of top/bottom should be >0.3 (not embedded in geometry)
    if result["d_top"] < 0.15 and result["d_bottom"] < 0.15:
        ok = 0
    # Mean luma proxy: if all ROIs are near-zero depth, camera is embedded
    if result["d_mid"] < 0.30 and result["d_top"] < 0.30:
        ok = 0
    result["ok"] = ok
    return result


def _compute_chase_pitch(chase_offset: List[float], target_local: List[float]) -> float:
    """Compute pitch for chase camera to look at target_local from chase_offset."""
    dx = target_local[0] - chase_offset[0]  # forward dist
    dz = target_local[2] - chase_offset[2]  # height diff
    horiz = math.sqrt(dx * dx + (target_local[1] - chase_offset[1]) ** 2)
    return math.degrees(math.atan2(dz, horiz)) if horiz > 0.01 else -25.0
...
def main() -> int:
    ...
    ap.add_argument("--out_dir", default="runs/topo_mvp/v35d_topo_to_nav2")
    ...
    # v35d camera parameters
    v35d_cam_height = float(os.environ.get("V35C_CAM_HEIGHT_M", "1.55"))
    v35d_cam_pitch = float(os.environ.get("V35C_CAM_PITCH_DEG", str(args.cam_pitch_deg)))
    v35d_chase_offset = [-2.0, 0.0, 1.6]
    v35d_chase_target = [0.5, 0.0, 1.2]  # look-at target in robot frame
    v35d_chase_pitch = _compute_chase_pitch(v35d_chase_offset, v35d_chase_target)
    v35d_fp_offset = [0.20, 0.0, v35d_cam_height]
    # Yaw smoothing state
    v35d_yaw_tau_s = 0.5
    v35d_yaw_clamp_degps = 90.0
    v35d_smooth_yaw = None  # will be initialised on first pose
    ...
    # v35d: Override camera offset to human height BEFORE backend init
    os.environ["ISAAC_CAMERA_OFFSET"] = f"{v35d_fp_offset[0]:.2f},{v35d_fp_offset[1]:.2f},{v35d_fp_offset[2]:.2f}"
    ...
    # v35d FP camera mount anchors
    print(
        f"[V35D_FP_MOUNT] offset_m=({v35d_fp_offset[0]:.2f},{v35d_fp_offset[1]:.2f},{v35d_fp_offset[2]:.2f}) "
        f"rpy_deg=(0.0,{v35d_cam_pitch:.1f},0.0) ok=1",
        flush=True,
    )
    # v35d chase camera anchors  (look-at based pitch)
    print(
        f"[V35D_CHASE] offset_m=({v35d_chase_offset[0]:.1f},{v35d_chase_offset[1]:.1f},{v35d_chase_offset[2]:.1f}) "
        f"target_m=({v35d_chase_target[0]:.1f},{v35d_chase_target[1]:.1f},{v35d_chase_target[2]:.1f}) "
        f"pitch_deg={v35d_chase_pitch:.1f} ok=1",
        flush=True,
    )
    print(
        f"[V35D_YAW_SMOOTH] tau_s={v35d_yaw_tau_s:.1f} clamp_degps={v35d_yaw_clamp_degps:.0f} ok=1",
        flush=True,
    )
    ...
    # v35d: Depth-based 3-ROI view sanity gate (replaces v35c 2-ray gate)
    sanity_ok = 0
    pitch_candidates = [v35d_cam_pitch, -20.0, -15.0, -5.0]
    for attempt, pitch_try in enumerate(pitch_candidates):
        backend.set_camera_mount(offset_m=v35d_fp_offset, rpy_deg=[0.0, pitch_try, 0.0], emit_anchor=False)
        obs = backend._build_obs()
        vs = _depth_view_sanity(obs, label="fp")
        # Also get mount vectors for V35D_FP_VEC anchor
        mount_info = dict(backend.get_camera_mount_info())
        print(
            f"[V35D_VIEW_SANITY] attempt={attempt} pitch_deg={pitch_try:.1f} "
            f"finite_ratio={vs['finite_ratio']:.3f} d_top={vs['d_top']:.3f} "
            f"d_mid={vs['d_mid']:.3f} d_bottom={vs['d_bottom']:.3f} ok={vs['ok']}",
            flush=True,
        )
        if vs["ok"] == 1:
            sanity_ok = 1
            v35d_cam_pitch = pitch_try
            # Print FP vector anchor
            cf = mount_info.get("cam_forward_world", [0, 0, 0])
            cu = mount_info.get("cam_up_world", [0, 0, 0])
            print(
                f"[V35D_FP_VEC] cam_forward_world=({cf[0]:.3f},{cf[1]:.3f},{cf[2]:.3f}) "
                f"cam_up_world=({cu[0]:.3f},{cu[1]:.3f},{cu[2]:.3f}) "
                f"angle_to_world_up_deg={mount_info.get('angle_to_world_up_deg', 0):.1f} "
                f"angle_to_robot_fwd_deg={mount_info.get('angle_to_robot_fwd_deg', 0):.1f}",
                flush=True,
            )
            break
    if sanity_ok != 1:
        print(f"[V35D_VIEW_SANITY] ok=0 reason=all_pitch_attempts_failed", flush=True)
    ...
    def _save_trace():
        if not pose_rows:
            return
        # v35d: Also write pose_trace_v35d.csv with dist_moved
        pose_csv = cap_dir / "pose_trace.csv"
        pose_csv_v35d = cap_dir / "pose_trace_v35d.csv"
        for csv_path, include_dist in [(pose_csv, False), (pose_csv_v35d, True)]:
            with csv_path.open("w", encoding="utf-8", newline="") as f:
                ...
                for row in pose_rows:
                    if include_dist:
                        ...
                        w.writerow(row[:8])
        print(
            f"[V35D_POSE_TRACE] ok=1 path={pose_csv_v35d} rows={len(pose_rows)}",
            flush=True,
        )
    ...
    # v35d: yaw smoothing
    v35d_smooth_yaw = float(pose0.yaw)
    # v35d: create chase capture dir
    ...
    for i in range(int(args.steps)):
        ...
        # v35d: yaw smoothing for FP rendering (does NOT affect nav2 control)
        dt_frame = float(cfg.get("dt_action", 0.25))
        alpha = 1.0 - math.exp(-dt_frame / max(0.01, v35d_yaw_tau_s))
        target_yaw = float(pose.yaw)
        diff = _wrap_pi(target_yaw - v35d_smooth_yaw)
        max_delta = math.radians(v35d_yaw_clamp_degps) * dt_frame
        diff = max(-max_delta, min(max_delta, diff))
        v35d_smooth_yaw = _wrap_pi(v35d_smooth_yaw + alpha * diff)
        ...
        overlay = _draw_overlay(
            ...
            [
                ...
                f"dist={moved:.2f}m yaw_s={math.degrees(v35d_smooth_yaw):.1f}",
            ],
        )
        ...
        # v35d: Chase camera capture (every 3rd step; look-at based pitch)
        if i % 3 == 0:
            saved_offset = list(v35d_fp_offset)
            saved_rpy = [0.0, v35d_cam_pitch, 0.0]
            backend.set_camera_mount(offset_m=v35d_chase_offset, rpy_deg=[0.0, v35d_chase_pitch, 0.0], emit_anchor=False)
            chase_obs = backend._build_obs()
            # v35d: chase view sanity check on first capture
            if i == 0:
                cs = _depth_view_sanity(chase_obs, label="chase")
                chase_mount = dict(backend.get_camera_mount_info())
                ccf = chase_mount.get("cam_forward_world", [0, 0, 0])
                ccu = chase_mount.get("cam_up_world", [0, 0, 0])
                print(
                    f"[V35D_CHASE_VEC] cam_forward_world=({ccf[0]:.3f},{ccf[1]:.3f},{ccf[2]:.3f}) "
                    f"cam_up_world=({ccu[0]:.3f},{ccu[1]:.3f},{ccu[2]:.3f})",
                    flush=True,
                )
                print(
                    f"[V35D_VIEW_SANITY] attempt=chase finite_ratio={cs['finite_ratio']:.3f} "
                    f"d_top={cs['d_top']:.3f} d_mid={cs['d_mid']:.3f} d_bottom={cs['d_bottom']:.3f} "
                    f"ok={int(cs['finite_ratio'] >= 0.95)}",
                    flush=True,
                )
    ...
    # v35d: Write trace on exit (handled by _save_trace)
    _save_trace()
