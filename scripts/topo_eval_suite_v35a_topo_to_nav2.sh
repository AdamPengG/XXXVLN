...
mkdir -p "${LOG_DIR}" "${MAP_DIR}" "${ROOMQA_DIR}" "${CAP_DIR}"
SUITE_LOG="${LOG_DIR}/topo_eval_suite_v35a_topo_to_nav2.log"
: > "${SUITE_LOG}"
...
export V34D_NAV2_CONTAINER="${V34D_NAV2_CONTAINER:-v35a_nav2_stack}"
export V34I_ODOM_TF_CONTAINER="${V34I_ODOM_TF_CONTAINER:-v35a_odom_tf_bridge}"
...
fail_suite() {
  local reason="$1"
  echo "[V35A_SUITE_OK] ok=0 rooms=${ROOM_COUNT:-0} doors=${DOOR_COUNT:-0} staged_succeeded=${STAGED_SUCCEEDED:-0} root=${OUT_ROOT} reason=${reason}" | tee -a "${SUITE_LOG}"
  exit 2
}
...
run_stage_task() {
  ...
  docker exec "${V34D_NAV2_CONTAINER}" bash -lc \
    "source /opt/ros/humble/setup.bash && export ROS_DOMAIN_ID=${ROS_DOMAIN_ID} RMW_IMPLEMENTATION=${RMW_IMPLEMENTATION} && python3 /home/peng/DualVLN/scripts/nav2/send_staged_goals_v35a.py --goals_json ${goals_json} --frame map --timeout_s ${V35A_SEG_TIMEOUT_S:-90} --dist_tol_m ${V35A_SEG_DIST_TOL_M:-0.45} --yaw_tol_deg ${V35A_SEG_YAW_TOL_DEG:-40} --result_json ${seg_json}" \
    > "${seg_log}" 2>&1 || true
  cat "${seg_log}" | tee -a "${SUITE_LOG}"

  if rg -q "\[V35A_NAV2_STAGED\].*ok=1" "${seg_log}"; then
    STAGED_SUCCEEDED=$((STAGED_SUCCEEDED + 1))
  else
    echo "[V35A_TASK_FAIL] task_id=${task_id} task=${task}" | tee -a "${SUITE_LOG}"
  fi
}

echo "[V35A_SUITE] start=$(date -Iseconds) root=${OUT_ROOT}" | tee -a "${SUITE_LOG}"
...
echo "[V35A_STAGE] official_usd=${STAGE_OFFICIAL}" | tee -a "${SUITE_LOG}"
...
# 2) Initial room/topo extraction from occupancy.
...
print(len(obj.get('rooms',[])))
PY
)"
DOOR_COUNT="$(python3 - <<'PY' "${ROOMQA_DIR}/doorway_edges.json"
import json,sys
obj=json.load(open(sys.argv[1],encoding='utf-8'))
print(len(obj.get('edges',[])))
PY
)"

# 3) Start Isaac ROS2 bridge probe.
READY_FILE="${OUT_ROOT}/bridge_ready_v35a.json"
rm -f "${READY_FILE}"
: > "${LOG_DIR}/isaac_probe_v35a.log"
(
  bash scripts/gpu/run_isaac_on_5090.sh -- \
    env ISAAC_HEADLESS="${V35A_HEADLESS:-1}" ROS_DOMAIN_ID="${ROS_DOMAIN_ID}" RMW_IMPLEMENTATION="${RMW_IMPLEMENTATION}" \
      ...
      --stage "${ISAAC_STAGE_USD}" \
      --out_dir "${OUT_ROOT}" \
      --steps "${V35A_CAPTURE_STEPS:-6000}" \
      --ready_file "${READY_FILE}" \
      --headless "${V35A_HEADLESS:-1}" \
      --idle_action "${V35A_IDLE_ACTION:-stop}" \
      --cam_pitch_deg "${ISAAC_CAMERA_PITCH_DEG}"
) > "${LOG_DIR}/isaac_probe_v35a.log" 2>&1 &
PROBE_PID=$!
...
if [ ! -f "${READY_FILE}" ]; then
  cat "${LOG_DIR}/isaac_probe_v35a.log" >> "${SUITE_LOG}" || true
...
# 5) Sample reachable goal anchors for deterministic staged tasks.
REACHABLE_JSON="${ROOMQA_DIR}/reachable_goals_seed.json"
SAMPLE_LOG="${LOG_DIR}/sample_reachable_goals_v35a.log"
: > "${SAMPLE_LOG}"
docker exec "${V34D_NAV2_CONTAINER}" bash -lc \
  "source /opt/ros/humble/setup.bash && export ROS_DOMAIN_ID=${ROS_DOMAIN_ID} RMW_IMPLEMENTATION=${RMW_IMPLEMENTATION} && python3 /home/peng/DualVLN/scripts/nav2/sample_reachable_goals_v34j.py --map_yaml ${V34D_MAP_YAML} --out_json ${REACHABLE_JSON} --count 6 --radius_min ${V35A_RADIUS_MIN:-0.8} --radius_max ${V35A_RADIUS_MAX:-1.8} --seed ${V35A_GOAL_SEED:-3}" \
  > "${SAMPLE_LOG}" 2>&1 || true
...
# 5) Resolve staged tasks and send staged goals.
...
# 6) Stop probe to flush pose/capture logs.
if [ -n "${PROBE_PID}" ] && kill -0 "${PROBE_PID}" >/dev/null 2>&1; then
  kill "${PROBE_PID}" >/dev/null 2>&1 || true
  wait "${PROBE_PID}" >/dev/null 2>&1 || true
  PROBE_PID=""
...
# 7) Re-run room/topo extraction with trajectory assignment.
...
# 8) Capture motion gate.
if ! run_and_log bash scripts/tools/check_capture_motion.sh --capture_dir "${CAP_DIR}" --tag v35a_topo_to_nav2 --write_gif 1; then
  fail_suite "capture_motion_failed"
fi

if [ "${STAGED_SUCCEEDED}" -lt 3 ]; then
  fail_suite "staged_goals_failed_${STAGED_SUCCEEDED}_of_3"
fi

echo "[V35A_SUITE_OK] ok=1 rooms=${ROOM_COUNT} doors=${DOOR_COUNT} staged_succeeded=${STAGED_SUCCEEDED} root=${OUT_ROOT}" | tee -a "${SUITE_LOG}"
